...
# Optimized for use with version 3.5.17-st of the Wrapper.

# Application
APP_NAME="${debian.package.name}"
APP_LONG_NAME="${debian.package.name}"

# Wrapper
WRAPPER_CMD="/opt/searchmetrics/${debian.package.name}/wrapper/bin/wrapper"
WRAPPER_CONF="/etc/searchmetrics/${debian.package.name}/startup.conf"
...
RUN_AS_USER=${debian.user}

# Location of the pid file.
PIDDIR="/var/run/searchmetrics"

if [ ! -d "$PIDDIR" ]
then
	mkdir -p $PIDDIR
	chown $RUN_AS_USER $PIDDIR
fi
...
console() {
    ...
    then
        ...
        # The string passed to eval must handles spaces in paths correctly.
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.name=\"$APP_NAME\" wrapper.displayname=\"$APP_LONG_NAME\" $ANCHORPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=${dependency.servicewrapper.version} $ADDITIONAL_PARA"
        eval $COMMAND_LINE
    else
        eval echo `gettext '$APP_LONG_NAME is already running.'`
        exit 1
